                          ...
                          {isLoading ? (
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                              <div className="space-y-2 text-xs text-slate-700">
                                ...
                                <p>School: {details.profile.school || 'n/a'}</p>
                                <p>Majors: {details.profile.majors.join(', ') || row.major_label}</p>
                                <p>Year: {details.profile.year || 'n/a'}</p>
                                <p>
                                  Availability: {details.profile.availabilityStartMonth || 'n/a'} /{' '}
                                  {details.profile.availabilityHoursPerWeek ? `${details.profile.availabilityHoursPerWeek}h/wk` : 'n/a'}
                                </p>
                                <p>Experience level: {details.profile.experienceLevel || 'n/a'}</p>
                                <p>Resume present: {details.profile.resumePresent ? 'Yes' : 'No'}</p>
                                <p>Profile completeness: {details.profile.profileCompletenessPercent}%</p>
                              ...
                              <div className="space-y-2 text-xs text-slate-700">
                                ...
                                <div className="pt-2">
                                  <div className="flex flex-wrap gap-2">
                                    ...
                                    {details.quickActions.viewFullProfileHref ? (
                                      <Link
                                        href={details.quickActions.viewFullProfileHref}
                                        className="rounded-md border border-slate-300 bg-white px-3 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50"
                                      >
                                        View full profile
                                      </Link>
                                    ) : (
                                      <span className="rounded-md border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-500">
                                        Full profile route unavailable
                                      </span>
                                    )}
                                  </div>
                                </div>
